...
val TempChannelToggleCommand = createCommand("Toggle Temporary Voice Channels") {
    ...
    onEvent {
        ...
        listen<CategoryDeleteEvent> {
            if (category.longID == sql { Guilds.select { Guilds.id eq guild.longID }.first()[Guilds.temp_category] }) {
...
val TempChannelsCommand = createCommand("Temporary Voice Channels") {
...
private class CheckTempChannelsTask(val client: IDiscordClient) : Runnable {
