...
class R8BallCommand: Command("8 Ball", "8ball", "8") {
    ...
    override fun execute(cmdUsed: String, args: Array<String>, user: IUser, message: IMessage, channel: IChannel, permission: Permission): MetadataMessageBuilder? {
        return MetadataMessageBuilder(channel).withContent(R8Ball().response)
    }
    ...
    class R8Ball {
        ...
        init {
            response = try {
                Unirest.get("https://apis.rtainc.co/twitchbot/8ball").asString().body
            } catch (e: UnirestException) {
                e.printStackTrace()
                "8ball api error, please report this to the developer"
            }
        }
    }
}