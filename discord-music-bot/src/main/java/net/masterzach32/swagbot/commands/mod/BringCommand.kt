...
class BringCommand: Command("Bring Users", "bring", permission = Permission.MOD) {
    ...
    override fun execute(cmdUsed: String, args: Array<String>, user: IUser, message: IMessage, channel: IChannel, permission: Permission): MetadataMessageBuilder? {
        ...
        val channel = message.author.connectedVoiceChannels[0]
        for(u in message.guild.users.filter { it.connectedVoiceChannels.size == 1 })
            u.moveToVoiceChannel(channel)
        return MetadataMessageBuilder(channel).withContent("Moved everyone to **$channel**.")
    }
